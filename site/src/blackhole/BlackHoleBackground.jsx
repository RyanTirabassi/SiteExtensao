...
const BlackHoleBackground = () => {
  ...
  useEffect(() => {
    ...
    // Salva a referência do container no início do effect
    ...
    const BLACK_HOLE_RADIUS = 0.4;
    const DISK_INNER_RADIUS = BLACK_HOLE_RADIUS + 0.1;
    const DISK_OUTER_RADIUS = 3.2;
    ...
    const lensingShader = {
      uniforms: {
        ...
        lensingStrength: { value: 0.08 },
        lensingRadius: { value: 0.2 },
      ...
      fragmentShader: `
        ...
        void main() {
          float normalizedRadius = smoothstep(${DISK_INNER_RADIUS.toFixed(
            3
          )}, ${DISK_OUTER_RADIUS.toFixed(3)}, vRadius);

          float spiral = vAngle * 3.0 - (1.0 / (normalizedRadius + 0.1)) * 2.0;
          vec2 noiseUv = vec2(vUv.x + uTime * uFlowSpeed * (2.0 / (vRadius * 0.3 + 1.0)) + sin(spiral) * 0.1,
    ...
    // Posição fixa - centralizado no quadrado vermelho
    function updateBlackHolePosition() {
      const ndc = new THREE.Vector3(0.45, 0.25, 0);
      const world = ndc.clone().unproject(camera);
      const dir = world.sub(camera.position).normalize();
      const DIST = 8;
      const BH_POS = camera.position.clone().add(dir.multiplyScalar(DIST));

      blackHoleMesh.position.copy(BH_POS);
      eventHorizon.position.copy(BH_POS);
      accretionDisk.position.copy(BH_POS);
    }
    ...
    function animate() {
      ...
      // Atualiza posição na tela para shader de lensing
      ...
      // Mantém o buraco negro na posição fixa (ANTES da rotação)
      updateBlackHolePosition();

      // Rotação contínua do buraco negro (DEPOIS da posição)
      const rotationSpeed = elapsedTime * 0.3;
      blackHoleMesh.rotation.y = rotationSpeed;
      eventHorizon.rotation.y = rotationSpeed;
      
      // IMPORTANTE: Manter a rotação X do disco fixa e apenas girar no Z
      accretionDisk.rotation.x = Math.PI / 2; // Sempre horizontal
      accretionDisk.rotation.z = rotationSpeed * 1.5;
  ...
  return <div ref={containerRef} className="blackhole-container" />;
};
